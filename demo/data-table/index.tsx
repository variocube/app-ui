...
function compareFruits(a: Fruit, b: Fruit, sortField: keyof Fruit, sortDirection: SortDirection) {
	...
	if (
		sortField == "id" || sortField == "density" || sortField == "calories" || sortField == "price"
		|| sortField == "stock"
	) {
...
function fakeSpringFetch(pageable: SpringPageable) {
	return new Promise<SpringPage<Fruit>>((resolve) => {
		setTimeout(() => {
			...
			const [sortField, sortDirection] = [...sort].pop()?.split(",") ?? [];
			resolve({
				content: fruits
...
export function SpringDataTable() {
	...
	// Passing the columns hides a persisted sort field that no sortable column matches, e.g. after a
	...
	return (
		<Box>
			...
			<Typography variant="body1" gutterBottom>
				...
				so that a persisted sort field of a column that is no longer sortable is hidden instead of being sent to
				the backend forever.
			</Typography>
			<DataTable
				header={
